...
class AugmentedConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride,
                 k, v, Nh, att_downsample, relative=True):
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, att_downsample=None):
...
class AAResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, layer_idx, stride=1):
        ...
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, att_downsample=att_downsample))
